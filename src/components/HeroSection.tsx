  ...
  return (
    <section id="top" className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div
        ...
        style={{ backgroundImage: `url(${swimmerBg})` }}
      />
      ...
      <div className="relative z-10 max-w-[900px] mx-auto px-8 pt-16 sm:pt-20 pb-12 sm:pb-16 text-center">
        ...
        <h1 className="text-white mb-6 sm:mb-8 leading-tight animate-fade-in text-[32px] sm:text-5xl md:text-6xl font-bold tracking-tight">
          {t('hero', 'main_headline', locale)}
        </h1>

        {/* Subheadline */}
        <p
          className="text-white text-xl sm:text-2xl mb-8 sm:mb-10 animate-fade-in leading-relaxed font-medium"
        ...
        {/* Profile Block */}
        <div
          className="flex flex-col items-center mb-8 sm:mb-10 animate-fade-in"
          style={{ animationDelay: '0.2s' }}
        >
          <div className="relative w-[130px] h-[130px] sm:w-[150px] sm:h-[150px] mb-4">
            <img
              src={gabrielProfile}
              alt={t('hero', 'profile_image_alt', locale)}
          ...
          <h2 className="text-white font-bold text-2xl sm:text-3xl mb-2">
            {t('hero', 'profile_name', locale)}
          </h2>
          <p className="text-white text-base sm:text-lg font-medium">
            {t('hero', 'profile_title', locale)}
          </p>
        </div>

        {/* Proof Line */}
        <div
          className="inline-flex flex-wrap items-center justify-center gap-y-3 gap-x-5 px-5 py-2.5 mb-8 sm:mb-10 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 shadow-lg animate-fade-in"
          style={{ animationDelay: '0.3s', letterSpacing: '0.5px' }}
        >
          <div className="flex items-center gap-2 text-white text-sm sm:text-base font-medium">
            <Award className="w-4 h-4 text-[#FFD700]" />
            <span>{t('hero', 'proof_olympian', locale)}</span>
          </div>
          <div className="hidden sm:block w-px h-4 bg-white/30" />
          <div className="flex items-center gap-2 text-white text-sm sm:text-base font-medium">
            <Globe className="w-4 h-4 text-[#FFD700]" />
            <span>{t('hero', 'proof_readers', locale)}</span>
          </div>
          <div className="hidden sm:block w-px h-4 bg-white/30" />
          <div className="flex items-center gap-2 text-white text-sm sm:text-base font-medium">
            <TrendingUp className="w-4 h-4 text-[#FFD700]" />
            <span>{t('hero', 'proof_raised', locale)}</span>
          </div>
        </div>
